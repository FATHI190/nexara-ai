...
async function createNewChat() {
    try {
        const response = await fetch('/api/new_session', { method: 'POST', headers: { 'Content-Type': 'application/json' } });
        const data = await response.json();
        if (data && data.id) {
            currentConversationId = data.id;
...
function appendTypingIndicator() {
    ...
    wrapper.innerHTML = `<img src="/static/logo-removebg-preview.png?v=4" class="avatar-img bot-av" alt="Nexara"><div class="message-box"><i class="fa-solid fa-circle-notch fa-spin"></i> يفكر...</div>`;
    chatContainer.appendChild(wrapper);
    setTimeout(() => { chatContainer.scrollTop = chatContainer.scrollHeight; }, 100);
    return wrapper;
}
...
function toggleVoiceRecording() {
    if (!recognition) { alert('المتصفح لا يدعم ميزة التعرف الصوتي. يرجى استخدام Google Chrome أو Edge.'); return; }
    const voiceBtn = document.getElementById('voiceTool');
