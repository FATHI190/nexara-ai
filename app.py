...
import os
import json
import time  # <--- تم إضافة هذا السطر (لتأخير الرد)
...
# حفظ / تحميل الأوزان
# ======================================================================
WEIGHTS_FILE = "nexara_weights.json"
...
# ======================================================================
# تدريب أو تحميل الأوزان (الإطلاق السريع)
# ======================================================================
print("🔄 فحص الأوزان المحفوظة...")
weights = load_weights()

if weights:
    print("✅ تم تحميل الأوزان من الملف!")
    ...
    print("🚀 لم يتم العثور على أوزان، بدء التدريب من الصفر...")
...
print("🎉 عقل Nexara مستعد!")

# ======================================================================
# مسارات الويب (بدون تفكير، ردود مباشرة)
# ======================================================================
...
def predict():
    try:
        ...
        # 1. فص الويب
        if mode == 'web':
            final_response = f"🌐 [فص الويب]: قمت بالبحث الفوري في شبكة الإنترنت عن الحقيقة المتعلقة بـ '{user_input}'. البيانات محدثة لعام 2026!"

        # 2. فص الكود
        elif mode == 'code':
            code = "```python\ndef nexara_feature():\n    print('تنفيذ طلبك الخاص بـ: ' + user_input)\n    return True\n\nnexara_feature()\n```"
            final_response = f"💻 [فص المبرمج الذكي]: إليك شفرة برمجية مقترحة بناءً على طلبك:\n\n{code}"

        # 3. فص الحساب والرياضيات (ردود مباشرة)
        else:
            ...
            if math_match:
                ...
                final_response = f"🤖 النتيجة الرياضية المستنتجة ذكياً هي: {final_output}"
            ...
            # 4. فص الطقس والدراسة
                ...
                if len(nums) >= 2 and any(x in user_input for x in ['طقس', 'جو', 'دراسة', 'مذاكرة', 'لعب']):
                    ...
                    final_response = f"🤖 القرار المستنتج: {decision}"
                else:
                    # الرد الافتراضي
                    final_response = f"🤖 استقبلت رسالتك: '{user_input}'. أنا فص ذكي مخصص للعمليات الرياضية الأربع (مثال: 8*8)، أو الطقس والدراسة."

        # ======================================================================
        # الحل السحري (تأخير الخادم لإظهار مؤشر التفكير)
        # ======================================================================
        time.sleep(1.5)  # انتظر 1.5 ثانية قبل إرسال الرد
